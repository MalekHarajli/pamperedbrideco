...
(function () {
  ...
  // cap render resolution — behind the scrim 1.5x is indistinguishable from 2x
  // but roughly half the pixels to draw each frame → much smoother on phones.
  ...
  function loop() {
    ...
    // Only redraw the film when the visible frame/crossfade (or a newly loaded
    // frame) actually changes — so the canvas rests when you're not scrolling.
    var n = Math.max(2, images.length);
    var f = Math.min(1, eased / FILM_END) * (n - 1);
    var key = Math.floor(f) + '_' + Math.round((f % 1) * 40) + '_' + loaded;
    if (key !== lastKey) { draw(); lastKey = key; }
    // Only update the text/scrim overlays when the scroll position changes.
    if (Math.abs(target - lastP) > 0.0004) { updateOverlays(target); lastP = target; }
    requestAnimationFrame(loop);
  }
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(form.action, {
        ...
        if (res.ok && res.data && res.data.success) {
          ...
          note.textContent = (res.data && res.data.message) ? ('Error: ' + res.data.message) : 'Something went wrong — please email bookings@thepamperedbrideco.com.';
        }
      }).catch(function () {
        note.style.color = 'var(--greige)';
